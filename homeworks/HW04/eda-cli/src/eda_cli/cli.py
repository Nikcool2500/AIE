...
def report(
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Слишком много уникальных категорий: **{quality_flags['has_high_cardinality']}**\n")
        f.write(f"- Есть дубликаты в колонке id: **{quality_flags['duplicates']}**\n\n")
        ...
        if not top_cats:
            ...
            f.write(f"См. файлы в папке `top_categories/` (выведено топ-{top_k_categories} признаков).\n\n")

        f.write("## Гистограммы числовых колонок\n\n")
        f.write(f"См. файлы `hist_*.png` (максимум {max_hist_columns} числовых колонок).\n")
...
def sample(
    path: str = typer.Argument(..., help="Путь к CSV-файлу."),
    n: int = typer.Option(10, "--n", "-n", help="Количество строк для вывода."),
    sep: str = typer.Option(",", "--sep", help="Разделитель в CSV."),
    encoding: str = typer.Option("utf-8", "--encoding", help="Кодировка файла.")
) -> None:
    """
    Показать таблицу со случайной выборкой строк:
    - N случайных строк из датасета;
    - загрузка из файла с выбранным разделителем;
