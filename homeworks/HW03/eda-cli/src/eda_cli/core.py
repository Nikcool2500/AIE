...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, df: pd.DataFrame = None) -> Dict[str, Any]:
    ...
    if df is not None:
        flags["has_high_cardinality"] = has_high_cardinality_categoricals(summary)
        flags["duplicates"] = has_suspicious_id_duplicates(summary, df)
...
def has_high_cardinality_categoricals(
        summary: DatasetSummary,
        threshold: int = 40,
        threshold_in_proc: float = None
) -> bool:
    """
    Определяет категориальные признаки с очень большим числом уникальных значений
    threshold_in_proc: используйте значение от 0 до 1 (включительно или нет - решайте конечно сами)
    """

    if threshold_in_proc:
        threshold = int(summary.n_rows * threshold_in_proc)
...
def has_suspicious_id_duplicates(summary: DatasetSummary, df: pd.DataFrame) -> bool:
    """
    ...
    if len(df[id_col.name].unique()) == summary.n_rows:
        return False
